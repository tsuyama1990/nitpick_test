...
1. `get_repository_metrics(self, owner: str, repo: str) -> RepositoryMetrics`: This method targets the `/repos/{owner}/{repo}` endpoint. Its responsibility is to fetch the core repository information. Before returning the raw JSON dictionary, it must inspect the `response.status_code`. If a 404 is encountered, it must raise the `RepositoryNotFoundError`. If a 403 or 429 is encountered, it must raise the `RateLimitExceededError`.
2. `get_recent_commits(self, owner: str, repo: str, limit: int = 100) -> list[CommitHistory]`: This method targets the `/repos/{owner}/{repo}/commits` endpoint. It must include query parameters to enforce the limit (`per_page=100`). Similar to the metrics method, it must rigorously check status codes and translate HTTP errors into domain exceptions before returning the raw JSON array.

**Important Constraint:** The `GitHubClient` should instantiate the Pydantic models to ensure validation occurs at the ingestion boundary. It returns `RepositoryMetrics` and `list[CommitHistory]` objects, parsing the raw dicts immediately. This ensures strict typing at the boundary and adheres to Schema-First design requirements. This maintains a strict separation of concerns.
