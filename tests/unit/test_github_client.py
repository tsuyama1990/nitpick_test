...
def test_get_repository_metadata_404(client: GitHubClient, mocker: MockerFixture) -> None:
    ...
    mock_response.raise_for_status.side_effect = HTTPStatusError("404 Not Found", request=mocker.Mock(), response=mock_response)
...
def test_get_repository_metadata_401(client: GitHubClient, mocker: MockerFixture) -> None:
    ...
    mock_response.raise_for_status.side_effect = HTTPStatusError("401 Unauthorized", request=mocker.Mock(), response=mock_response)
...
def test_get_repository_metadata_403_rate_limit(client: GitHubClient, mocker: MockerFixture) -> None:
    mock_response = mocker.Mock(spec=Response)
    mock_response.status_code = 403
    mock_response.raise_for_status.side_effect = HTTPStatusError("403 Forbidden", request=mocker.Mock(), response=mock_response)
...
def test_get_repository_metadata_429(client: GitHubClient, mocker: MockerFixture) -> None:
    ...
    mock_response.raise_for_status.side_effect = HTTPStatusError("429 Too Many Requests", request=mocker.Mock(), response=mock_response)
...
def test_get_repository_metadata_connection_error(client: GitHubClient, mocker: MockerFixture) -> None:
    mocker.patch("httpx.Client.get", side_effect=RequestError("Connection Refused", request=mocker.Mock()))
