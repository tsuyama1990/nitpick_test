...
def test_get_repository_info_404(mock_settings: None, httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="https://api.github.com/repos/test/test-repo", status_code=404)
    client = GitHubClient()
...
def test_get_repository_info_403(mock_settings: None, httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="https://api.github.com/repos/test/test-repo", status_code=403)
    client = GitHubClient()
...
def test_get_repository_info_429(mock_settings: None, httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(url="https://api.github.com/repos/test/test-repo", status_code=429)
    client = GitHubClient()
...
def test_get_recent_commits_success(mock_settings: None, httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(
        ...
        json=[
            {
                "sha": "abc1234",
                "commit": {"committer": {"name": "Alice", "date": "2023-01-01T12:00:00Z"}},
            }
        ],
    )
    client = GitHubClient()
...
def test_get_repository_info_type_error(mock_settings: None, httpx_mock: HTTPXMock) -> None:
    httpx_mock.add_response(
        url="https://api.github.com/repos/test/test-repo", json=["not", "a", "dict"]
    )
    client = GitHubClient()
