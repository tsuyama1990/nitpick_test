...
def test_data_transformer_success() -> None:
    ...
    date_dict = dict(zip(by_date["date"].to_list(), by_date["commit_count"].to_list(), strict=False))
    assert date_dict["2023-01-01"] == 2
    assert date_dict["2023-01-02"] == 1

    assert len(top_users) == 2
    # Verify user aggregation
    user_dict = dict(zip(top_users["name"].to_list(), top_users["commit_count"].to_list(), strict=False))
