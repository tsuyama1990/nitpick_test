...
def aggregate_commits_per_day(commits: list[CommitInfo]) -> pl.DataFrame:
    if not commits:
        return pl.DataFrame({"date": [], "commit_count": []}, schema={"date": pl.String, "commit_count": pl.UInt32})
...
def get_top_committers(commits: list[CommitInfo]) -> pl.DataFrame:
    if not commits:
        return pl.DataFrame({"name": [], "commit_count": []}, schema={"name": pl.String, "commit_count": pl.UInt32})

    data = [{"name": c.name} for c in commits]
    df = pl.DataFrame(data)

    return df.group_by("name").len(name="commit_count").sort("commit_count", descending=True).head(5)
