...
class DataTransformer:
    def process_commits(self, commits: list[Commit]) -> tuple[pl.DataFrame, pl.DataFrame]:
        ...
        """
        if not commits:
            empty_date = pl.DataFrame({"date": [], "commit_count": []}, schema={"date": pl.String, "commit_count": pl.UInt32})
            empty_user = pl.DataFrame({"name": [], "commit_count": []}, schema={"name": pl.String, "commit_count": pl.UInt32})
        ...
        df_date = df.with_columns(
            pl.col("date").dt.date().cast(pl.String).alias("date_str")
        )
