...
def _commits_to_df(commits: list[CommitRecord]) -> pl.DataFrame:
    ...
    if not commits:
        return pl.DataFrame(schema={"date": pl.Datetime, "author_name": pl.Utf8, "message": pl.Utf8})
...
def transform_commits_to_top_committers(commits: list[CommitRecord], limit: int = 5) -> pl.DataFrame:
